...
library(XML)
#################################################
aurl="https://www.amazon.in/OnePlus-McLaren-Limited-Orange-Storage/product-reviews/B07DJCVTBH/ref=cm_cr_getr_d_paging_btm_prev_1?ie=UTF8&reviewerType=all_reviews&pageNumber"
...
for (i in 1:20){
  ...
  amezon_reviews=c(amezon_reviews,rev)
}
...
write.csv(amezon_reviews,file = "amezon_reviews.csv")
write.csv(amezon_reviews,file = "amezon_reviews.txt")
...
nqwrd=c("get","got","see","using","mobile","charge","dont","need","user","review","super","phone","one","oneplus")
nqwrd
...
df
windows()
barplot(sort(df),col = 1:8,horiz = T,xlab = "persentage of emotion",ylab = "emotions",main = "amezon product-analysis")

print(" amezon product sentimnet analsys is complted")
